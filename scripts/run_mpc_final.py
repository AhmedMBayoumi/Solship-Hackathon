"""Run MPC with our NEW BEST forecast (light-reg walkforward bagging, 60.83% NRMSE)."""
import sys, time
...
preds = pd.read_csv(ROOT / "outputs/forecasts/bagging_walkforward_FINAL_test_preds.csv",
...
print("MPC with NEW BEST forecast (60.83% NRMSE walkforward + light reg)\n")
...
for H in [4, 8, 16, 24, 48, 96]:
    ...
    if H == 96:
        out = ROOT / "outputs" / "mpc_final_H96.parquet"
...
prev_v2 = -18.79  # from previous walkforward bagging at H=96
new_h96 = next(r for r in results if r["H"] == 96)
print(f"\nProgression at H=96 (1-day battery cycle):")
print(f"  Heavy reg walkforward (prev submission): EUR -18.79")
print(f"  Light reg walkforward (THIS RUN)       : EUR {new_h96['total']:+.2f}")
print(f"  Improvement: EUR {new_h96['total'] - prev_v2:+.2f}")

pd.DataFrame(results).to_csv(ROOT / "outputs/horizon_sweep_final.csv", index=False)
